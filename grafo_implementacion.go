...
func CrearGrafoPesado[K comparable](dirigido bool) GrafoPesado[K] {
	return &grafo[K]{adyacencias: TDADICC.CrearHash[K, TDADICC.Diccionario[K, int]](), dirigido: dirigido}
}
...
func (g *grafo[K]) EsVertice(v K) bool {
	return g.adyacencias.Pertenece(v)
}

func (g *grafo[K]) AgregarVertice(vertice K) {
	if g.adyacencias.Pertenece(vertice) {
		panic("El vertice ya pertenece al grafo")
	}
	g.adyacencias.Guardar(vertice, TDADICC.CrearHash[K, int]())
}
...
func (g *grafo[K]) ObtenerVertices() []K {
	res := []K{}
	for iter := g.adyacencias.Iterador(); iter.HaySiguiente(); iter.Siguiente() {
		v, _ := iter.VerActual()
		res = append(res, v)
	}
	return res
